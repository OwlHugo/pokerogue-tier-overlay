...
describe('nomes de bioma', () => {
  ...
  test('traz os nomes traduzidos, nao os identificadores do enum', () => {
    expect(BIOME_NAMES[0]).toBe('Cidade');
    expect(BIOME_NAMES[1]).toBe('Planície');
  });

  test('as chaves sao numericas', () => {
    for (const key of Object.keys(BIOME_NAMES)) expect(key).toMatch(/^\d+$/);
  });
});

describe('nomes de ability', () => {
  test('cobre pelo menos 250 abilities', () => {
    expect(Object.keys(ABILITY_NAMES).length).toBeGreaterThanOrEqual(250);
  });

  test('resolve a hidden ability do Ralts, lida ao vivo do jogo', () => {
    expect(ABILITY_NAMES[140]).toBe('Telepathy');
  });

  test('as chaves sao numericas e positivas', () => {
    for (const key of Object.keys(ABILITY_NAMES)) {
...
describe('ambas as tabelas', () => {
  test('nenhum nome vazio', () => {
    for (const table of [ABILITY_NAMES, BIOME_NAMES]) {
